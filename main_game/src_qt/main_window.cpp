...
void main_window::tick_god_with_monster() {
    if (damn->game->alien.get_state() == eclipse::Not_on_the_field) {
        ...
        cur_enemy = asteroids;
        damn->ready_for_moving_alien = false;
        timer_for_ticks->start(1000 / ticks_for_second);
        return;
    }

    if (!damn->ready_for_moving_alien && damn->check_field_and_set_monster()) {
        damn->ready_for_moving_alien = true;
    }
    if (damn->ready_for_moving_alien) {
